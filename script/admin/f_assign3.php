...
<nav>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['assign'])) {
      ...
      // Check if a faculty is already assigned to this subject in the same semester and department
        $check_query = "SELECT 1 FROM assign_fact2 
        WHERE sub_code = '$subject_code' 
        AND sub_sem = '$semester_assign' 
        AND sub_dept = '$subject_department'
        AND a_year = '$year'";

        $check_result = mysqli_query($con, $check_query);

        if (mysqli_num_rows($check_result) > 0) {
        echo "<p>Error: A faculty is already assigned to this subject for the selected semester and department.</p>";
        } else {
        // Insert data into fact_assign table if no faculty is already assigned
        $query = "INSERT INTO assign_fact2 (f_id, sub_code, sub_sem, sub_dept, a_year) 
        VALUES ('$faculty_id', '$subject_code', '$semester_assign', '$subject_department', '$year')";
        $result = mysqli_query($con, $query);
...
//Remove Faculty assingment
if (isset($_GET['delete_id'])) {
    $delete_id = $_GET['delete_id'];
    $query = "DELETE FROM assign_fact2 WHERE f_id = '$delete_id'";
    if (mysqli_query($con, $query)) {
        echo "<script>alert('Assignment deleted successfully'); window.location.href='fact_assign.php';</script>";
    } else {
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)): ?>
                <tr>
                    ...
                    <td><a href="?delete_id=<?php echo $row['f_id']; ?>" onclick="return confirm('Are you sure you want to delete this assignment?');">REMOVE</a></td>
                </tr>
            <?php endwhile; ?>
        </tbody>
    </table>
